 ...
 */
class CrashReportingPreferenceWithDefault(key:String, defaultValue:(String) -> Crashreporting):PreferenceTypeWithDefault<SharedPreferences, Crashreporting>(key, defaultValue) {
    override fun getValue(
        thisRef: TypedPreferences<SharedPreferences>,
        property: KProperty<*>
    ): Crashreporting {
        return if(thisRef.sharedPreferences.contains(key)) {
            Crashreporting.values().find {
                it.value == thisRef.sharedPreferences.getString(key, "")
            } ?: defaultValue(key)
